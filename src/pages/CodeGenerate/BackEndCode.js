...
import styles from './BackEndCode.less';
      ...
      >
        <Form>
          ...
          <FormItem
            {...formItemLayout}
            label="状态">
            {getFieldDecorator('status', {initialValue: formVals.status + ""})(
              <Select style={{width: '100%'}}>
...
@Form.create()
class BackEndCode extends PureComponent {
  state = {
    modalVisible: false,
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      if (group && group.length !== 0) {
        values.group = group[group.length - 1] || null;
      }
      ...
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{md: 8, lg: 24, xl: 48}}>
          ...
          <Col md={6} sm={24}>
            <FormItem label="用户组">
              {getFieldDecorator('group')(
                <Cascader options={this.props.user.groups} placeholder="选择用户组"/>,
              )}
            </FormItem>
          </Col>
          <Col md={6} sm={24}>
